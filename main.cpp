...
class Road : public PhysicalObject{
    ...
    int totalLanes; // between [1, totalLanes]
...
class Vehicle{
  ...
  int speed; // meters per seconds.
  Road *road;
  int current_lane = 0;

  Vehicle(Road rd): road(rd) {
  ...
  void observeAndDrive() { // Change Lanes
    // Here I need to check the car's distance from other cars, 
    // and need to change lanes, if required.
...
int main(){
    ...
    std::thread t1(&Vehicle::Run, &Car, 1, 200);
    t1.join();
}
